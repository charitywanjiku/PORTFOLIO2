      ...
      <div className="container portfolio__container">
        ...
        {Array.from({ length: numRows }).map((_, rowIndex) => (
          ...
          <div className="portfolio__row" key={rowIndex}>
            ...
            {Array.from({ length: itemsPerRow }).map((_, itemIndex) => {
              const dataIndex = rowIndex * itemsPerRow + itemIndex;
              
              if (dataIndex < portfolioData.length) {
                const { id, image, tecs, title, resume, github, demo } = portfolioData[dataIndex];
                return (
                  <PortfolioItem 
                    key={id} 
                ...
                return null; // or any other handling you want to do when the index is out of bounds
              }
